...
function findThreeLargestNumbers(array) {
	...
	while (largestNumsArr.length !== 3) {
		// O(3) -> Worst Case: Maximum of 3 loops
		let numsFound = largestNumsArr.length;
		let largestNumIndex = 0;
		let largestNum = null;

		for (let i = 0; i < array.length; i++) {
			// O(n) -> Worst Case: Iterates through entire array once.
			...
			if (i === array.length - 1) {
				...
				if (array[array.length - 1] !== largestNum) {
					// Check if the largest number is in the last position
